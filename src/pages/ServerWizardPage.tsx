...
export function ServerWizardPage() {
  ...
  const [selectedISPTemplate, setSelectedISPTemplate] =
  ...
  const [stepStatus, setStepStatus] = useState<{
    [key: string]: 'incomplete' | 'complete';
  }>({
  ...
  const steps: WizardStep[] = [
    { id: 'introduction', title: 'Introduction', status: 'complete' },
    {
      ...
      status: stepStatus['server-details'],
    },
    {
      id: 'domains',
      title: 'Domain Configuration',
      status: stepStatus['domains'],
    },
    { id: 'pool-types', title: 'Pool Types', status: stepStatus['pool-types'] },
    {
      id: 'target-isps',
      title: 'Target ISPs',
      status: stepStatus['target-isps'],
    },
  ...
  const handleNext = async () => {
    ...
    // Skip server update for introduction step
    ...
    if (currentStep === 'server-details') {
      ...
      try {
        ...
        if (connectResponse.session_id) {
          const newNodeId = connectResponse.node_id;
          const newSessionId = connectResponse.session_id;
          ...
          // Update server data using the values we just received
          const cleanedPoolTypes = poolTypeConfigs.map(
            ({ template, ...rest }) => rest
          );

          const serverData = {
            nodeId: newNodeId,
            ip_addresses: ipResponse.ip_addresses,
            domains: domains,
            pool_types: cleanedPoolTypes,
          };

          await axiosPut(`/api/v1/server`, serverData);
    ...
    // Handle updates for other steps
    try {
      const cleanedPoolTypes = poolTypeConfigs.map(
        ({ template, ...rest }) => rest
      );
  ...
  const handlePoolTemplateChange = (
    poolType: string,
  ...
  const removePoolType = (type: string) => {
    ...
    setPoolTypeConfigs((prev) =>
      prev.filter((config) => config.pool_type !== type)
    );
  };
  ...
  const handleCreateServer = async () => {
    try {
      ...
      if (response.success) {
        ...
        toast.dismiss(toastId);
        toast.error(
          response.error || 'Failed to setup server. Please try again.'
        );
      }
    } catch (error) {
  ...
  const renderIntroduction = () => (
    <div className="max-w-2xl">
      ...
      <p className="text-gray-600 mb-6">
        This wizard will guide you through setting up a new PMTA server
              ...
              <div className="flex items-center gap-2">
                ...
                <TemplateSelector
                  ...
                  onTemplateChange={(template) =>
                    handlePoolTemplateChange(config.pool_type, template)
                  }
                />
              </div>
            </div>
  ...
  const renderTargetISPs = () => (
    <div className="max-w-4xl">
      ...
      <Tabs defaultActiveKey={poolTypes[0]} className="mb-4">
        {poolTypeConfigs.map((config) => (
          <Tab
